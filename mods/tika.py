...
log_path = os.getenv('TIKA_LOG_PATH', os.path.join('cfg', 'tika'))
...
EncodeUtf8 = 0
csvOutput = 0
t = False #Added to display download bar
...
def killServer():
    ...
    if TikaServerProcess:
        # removed try-except block due to incorrect logic
...
def getRemoteJar(urlOrPath, destPath):
    ...
    if urlp.scheme == '':
        ...
        log.info('Retrieving %s to %s.' % (urlOrPath, destPath))
        try:
            urlretrieve(urlOrPath, destPath, reporthook=download_progress)
            global t
            t.close()
            t = False
        except IOError:
...
def download_progress(count, blocksize, totalSize):
    global t
    if not t:
        t = tqdm(total=totalSize, unit='B', unit_scale=True)
